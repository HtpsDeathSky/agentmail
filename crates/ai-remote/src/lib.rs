...
所有可读文本字段必须使用简体中文。
字段要求:
summary: string，使用 1 到 3 句简体中文总结邮件核心内容，让用户不看原文也能知道邮件在说什么、涉及谁、有什么结论、要求、金额、时间或截止点；不要只描述邮件类型。
...
mod tests {
    ...
    #[test]
    fn system_prompt_requires_content_summary_in_simplified_chinese_json() {
        assert!(SYSTEM_PROMPT.contains("简体中文"));
        assert!(SYSTEM_PROMPT.contains("核心内容"));
        assert!(SYSTEM_PROMPT.contains("不要只描述邮件类型"));
